...
MATH_PROMPT_SUFFIX = "Reason step by step and put your final answer in \\boxed{}."
...
def is_hf_dataset_source(data_file: str) -> bool:
    if not isinstance(data_file, str):
        return False
    if data_file.startswith("hf://"):
        return True
...
def parse_hf_dataset_source(data_file: str) -> HFDatasetSpec:
    if data_file.startswith("hf://"):
        parsed = urlparse(data_file)
        repo_id = f"{parsed.netloc}{parsed.path}".strip("/")
        params = parse_qs(parsed.query)
        config_name = _first_param(params, "config", "subset", "name")
        split = _first_param(params, "split") or "train"
        revision = _first_param(params, "revision", "ref")
    ...
    if len(path_parts) < 3 or path_parts[0] != "datasets":
        raise ValueError(f"Unsupported Hugging Face dataset source: {data_file}")
    ...
    if len(path_parts) >= 6 and path_parts[3] == "viewer":
        config_name = path_parts[4]
        split = path_parts[5]
    elif len(path_parts) >= 5 and path_parts[3] == "tree":
    ...
    config_name = config_name or _first_param(params, "config", "subset", "name")
    split = split or _first_param(params, "split") or "train"
    revision = revision or _first_param(params, "revision", "ref")
...
def _first_param(params: dict[str, list[str]], *keys: str) -> Optional[str]:
    for key in keys:
...
def _wrap_user_prompt(prompt_text: str) -> list[dict[str, str]]:
    return [{"role": "user", "content": prompt_text.strip()}]


def _extract_ground_truth(value) -> str:
    if value is None:
        raise ValueError("Missing ground truth value")
    if isinstance(value, (list, tuple)):
        for item in value:
            if item is not None and str(item).strip():
                return _extract_ground_truth(item)
        raise ValueError("Missing ground truth value")
    ...
    try:
        from verl.utils.reward_score.math_verify import preprocess_model_output

        processed = preprocess_model_output(text)
        if processed:
...
def _default_data_source(repo_id: str, config_name: Optional[str]) -> str:
    if repo_id == "openai/gsm8k":
        return "openai/gsm8k"
    if repo_id == "EleutherAI/hendrycks_math":
        if config_name:
            return f"DigitalLearningGmbH/MATH-lighteval/{config_name}"
        return "DigitalLearningGmbH/MATH-lighteval"
    if repo_id == "BytedTsinghua-SIA/DAPO-Math-17k":
        return "math_dapo"
    if repo_id == "JingzeShi/amc23":
        return "amc23"
    if repo_id.startswith("math-ai/aime"):
        return repo_id.split("/")[-1]
    return repo_id


def normalize_hf_row(row: dict, spec: HFDatasetSpec, config_name: Optional[str] = None) -> dict:
    config_name = config_name or spec.config_name
    data_source = row.get("data_source") or _default_data_source(spec.repo_id, config_name)
    prompt = row.get("prompt")
    prompt_suffix = MATH_PROMPT_SUFFIX

    if prompt is None:
        if spec.repo_id == "openai/gsm8k":
            question = _first_present(row, "question", "problem")
            if question is None:
                raise ValueError("GSM8K row is missing question/problem")
            prompt_suffix = GSM8K_PROMPT_SUFFIX
            prompt = _wrap_user_prompt(f"{str(question).strip()} {prompt_suffix}")
        else:
            question = _first_present(row, "problem", "question", "prompt")
            if question is None:
                raise ValueError(f"Unsupported HF row for {spec.repo_id}: missing prompt/problem/question")
            prompt = _wrap_user_prompt(f"{str(question).strip()} {prompt_suffix}")
    elif isinstance(prompt, str):
        prompt = _wrap_user_prompt(prompt)
    ...
    if ground_truth is None:
        raise ValueError(f"Unsupported HF row for {spec.repo_id}: missing answer/ground_truth")

    normalized_type = row.get("type") or config_name
    normalized = {
        ...
        "reward_model": {
            ...
            "ground_truth": _extract_ground_truth(ground_truth),
        },
        "extra_info": {
...
class RLHFDataset(Dataset):
    ...
    def _download(self, use_origin_parquet=False):
        ...
        for i, parquet_file in enumerate(data_files):
            if is_hf_dataset_source(parquet_file):
                self.data_files[i] = parquet_file
            else:
                self.data_files[i] = copy_to_local(src=parquet_file, cache_dir=self.cache_dir, use_shm=self.use_shm)

    def _load_hf_dataset(self, data_file: str) -> datasets.Dataset:
        spec = parse_hf_dataset_source(data_file)
        config_names = [spec.config_name]

        if spec.repo_id == "EleutherAI/hendrycks_math" and spec.config_name is None:
            config_names = datasets.get_dataset_config_names(spec.repo_id)
        ...
        for config_name in config_names:
            ...
            dataset = dataset.map(
                lambda row, config_name=config_name: normalize_hf_row(row, spec, config_name),
                remove_columns=dataset.column_names,
                desc=f"Normalizing {spec.repo_id}:{config_name or 'default'}[{spec.split}]",
            )
            dataframes.append(dataset)

        return dataframes[0] if len(dataframes) == 1 else datasets.concatenate_datasets(dataframes)
    ...
    def _read_files_and_tokenize(self):
        ...
        for parquet_file in self.data_files:
            ...
            if is_hf_dataset_source(parquet_file):
                dataframe = self._load_hf_dataset(parquet_file)
