...
def init_fn(x: torch.nn.Module):
    if torch.distributed.get_rank() != 0:
        # Get the local rank from environment variable or distributed rank
        # In Ray, CUDA_VISIBLE_DEVICES is set per worker, so local rank is typically 0
        local_rank = int(os.environ.get("LOCAL_RANK", 0))
        
        # Explicitly set the CUDA device before allocating memory
        # This is critical for avoiding "CUDA device busy" errors on B200 GPUs
        get_torch_device().set_device(local_rank)
        
        # Use device index directly instead of get_device_id() which may return wrong value
        x = x.to_empty(device=f"{get_device_name()}:{local_rank}", recurse=False)
