...
def initialize_global_process_group_ray(timeout_second=None):
    ...
    # Use a file lock to serialize CUDA initialization across processes on the same node
    # This prevents "CUDA device busy" errors on some GPU architectures (e.g., B200)
    lock_file = f"/tmp/verl_cuda_init_{getpass.getuser()}_{local_rank}.lock"
    ...
    try:
        ...
        # If lock acquisition fails, try without lock as fallback
        import logging
        logging.warning(f"Failed to acquire CUDA init lock: {e}, proceeding without lock")
        get_torch_device().set_device(local_rank)
