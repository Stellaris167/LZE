...
def _sanitize_dist_env_for_ray() -> None:
    """Remove stale torch distributed rendezvous env vars before ``ray.init``.

    When this script is launched from orchestrators that previously ran other
    distributed jobs, inherited env (MASTER_ADDR/PORT, RANK, etc.) can leak
    into vLLM worker startup and cause 30-minute DistStore timeouts.
    """
    vars_to_remove = {
        "MASTER_ADDR",
        "MASTER_PORT",
    ...
    for k in list(os.environ.keys()):
        if k in vars_to_remove or k.startswith("TORCHELASTIC_"):
            ...
            removed.append(k)
    if removed:
        print(f"[main_ppo] sanitized distributed env vars: {sorted(set(removed))}")
