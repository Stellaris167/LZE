...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Check if stop_reason is available in non_tensor_batch
    if "__stop_reason__" in batch.non_tensor_batch:
        stop_reasons = batch.non_tensor_batch["__stop_reason__"]
        # Create aborted mask based on stop_reason field
        aborted_mask = torch.tensor([sr == "aborted" for sr in stop_reasons], dtype=torch.bool, device=response_length.device)
    else:
        # Fallback: use response_length == 0 as before (for backward compatibility)
    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        ...
        for uid, var2vals in uid2var2vals.items():
            ...
            for var_name, var_vals in var2vals.items():
                ...
                # filter out None values to avoid numpy mean calculation errors
                ...
                if n_resps > 1:
                    ...
                    for n in ns:
                        ...
                        if has_pred:
                            # create vote_data
                            vote_data = [
                                {"val": val, "pred": pred} for val, pred in zip(var_vals_filtered, pred_vals, strict=True)
                            ]
                            # compute maj metrics
