...
# Force vLLM to use V1 engine - must be set before importing vllm modules
...
# vLLM 0.12.0+ moved FlexibleArgumentParser to vllm.utils.argparse_utils
...
# GrammarOutput only exists in vLLM >= 0.12.0
if _VLLM_VERSION >= version.parse("0.12.0"):
...
def _sanitize_dist_env_for_vllm() -> None:
    vars_to_remove = {
        "MASTER_ADDR",
        "MASTER_PORT",
        "RANK",
    ...
    for k in list(os.environ.keys()):
        if k in vars_to_remove or k.startswith("TORCHELASTIC_"):
...
class ExternalZeroMQDistributedExecutor(Executor):
    ...
    def _init_executor(self) -> None:
        ...
        kwargs = dict(
            ...
            # Let worker decide rank/local_rank based on parallel topology.
            # For TP>1 we must preserve unique env-derived ranks.
            local_rank=None,
            rank=None,
...
class vLLMHttpServer:
    ...
    def _release_reserved_ports(self):
        for attr in ("_master_sock", "_dp_master_sock"):
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None):
        ...
        # logprobs_mode is only supported in vLLM >= 0.12.0
        if _VLLM_VERSION >= version.parse("0.12.0") and self.config.logprobs_mode:
    ...
    async def wait_for_requests_to_drain(self):
        # wait_for_requests_to_drain is only available in vLLM >= 0.12.0
        if hasattr(self.engine, 'wait_for_requests_to_drain'):
            await self.engine.wait_for_requests_to_drain()
        else:
            # For older versions, just return (no draining needed for simple cases)
            pass
    ...
    async def abort_all_requests(self, reset_prefix_cache: bool = True) -> dict[str, Any]:
        ...
        """
        try:
            # Check if the engine has the required attributes (vLLM >= 0.12.0)
            if not hasattr(self.engine, 'output_processor') or not hasattr(self.engine, 'engine_core'):
                # For vLLM 0.11.x, use the simpler abort API
                if hasattr(self.engine, 'abort'):
                    # Note: In 0.11.x we don't have easy access to request IDs
                    logger.warning("abort_all_requests: vLLM 0.11.x does not support detailed abort tracking")
                    return {"aborted_count": 0, "request_ids": [], "note": "vLLM 0.11.x - limited abort support"}
                return {"aborted_count": 0, "request_ids": [], "error": "abort not supported in this vLLM version"}
    ...
    async def abort_request(self, request_id: str, reset_prefix_cache: bool = True) -> dict[str, Any]:
        ...
        try:
            # Check if the engine has the required attributes (vLLM >= 0.12.0)
            if not hasattr(self.engine, 'output_processor') or not hasattr(self.engine, 'engine_core'):
                # For vLLM 0.11.x, use the simpler abort API if available
                if hasattr(self.engine, 'abort'):
                    await self.engine.abort(request_id)
                    logger.info(f"Aborted request (vLLM 0.11.x): {request_id}")
                    return {"aborted": True, "request_id": request_id}
                return {"aborted": False, "request_id": request_id, "error": "abort not supported in this vLLM version"}
...
class vLLMReplica(RolloutReplica):
    ...
    async def launch_servers(self):
        ...
        print(f"[launch_servers] Starting, workers={len(self.workers)}, world_size={self.world_size}")
        assert len(self.workers) == self.world_size, (
            f"worker number {len(self.workers)} not equal to world size {self.world_size}"
        )

        # get node_id of all workers
        print("[launch_servers] Getting worker node IDs...")
        ...
        print(f"[launch_servers] Got worker node IDs: {worker_node_ids}")
        ...
        print(f"[launch_servers] Creating {nnodes} server actors...")
        for node_rank in range(nnodes):
            ...
            # Server actor is control-plane only. Real model execution is done in rollout workers
            # (ExternalZeroMQDistributedExecutor -> vLLMAsyncRollout workers), so server should not
            # reserve extra GPUs in any rollout mode.
            num_gpus_for_server = 0
            server_options = {
                "num_cpus": 0,
                "num_gpus": num_gpus_for_server,
                "runtime_env": {"env_vars": {"RAY_EXPERIMENTAL_NOSET_CUDA_VISIBLE_DEVICES": "1"}},
                "name": name,
            }
            # Keep strict node affinity for deterministic intra-node communication.
            server_options["scheduling_strategy"] = ray.util.scheduling_strategies.NodeAffinitySchedulingStrategy(
                node_id=node_id,
                soft=False,
            )
        ...
        print(f"[launch_servers] Created {len(self.servers)} server actors")

        # launch http server in each node
        print("[launch_servers] Getting master address...")
        try:
            master_address, master_port = await asyncio.wait_for(
                self.servers[0].get_master_address.remote(),
                timeout=180,
            )
        except asyncio.TimeoutError as e:
            raise RuntimeError(
                "Timeout waiting for vLLM server actor to return master address. "
                "This is usually a Ray scheduling deadlock (pending server actor)."
            ) from e
        print(f"[launch_servers] Master address: {master_address}:{master_port}, launching servers...")
        ...
        print("[launch_servers] All servers launched!")
        ...
        print(f"[launch_servers] Server address: {self._server_address}")
