...
class HFRollout(BaseRollout):
    ...
    async def resume(self, tags: list[str]):
        # HFRollout is bound to local module, no external engine state.
        return None
    ...
    async def update_weights(
        ...
        **kwargs,
        ...
        # HFRollout shares the in-process actor module; no extra weight sync needed.
        return None

    async def release(self):
        # HFRollout has no external server memory to release.
        return None

    def generate_sequences(self, prompts: DataProto) -> DataProto:
        batch_size = prompts.batch.batch_size[0]
        num_chunks = max(batch_size // self._cfg_get(self.config, "micro_batch_size", batch_size), 1)
    ...
    def _generate_minibatch(self, prompts: DataProto) -> DataProto:
        ...
        top_p = prompts.meta_info.get("top_p", self._cfg_get(self.config, "top_p", 1.0))
        top_k = max(
            0,
            prompts.meta_info.get("top_k", self._cfg_get(self.config, "top_k", 0)),
