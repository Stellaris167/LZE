...
from verl.workers.rollout.sglang_rollout.stopping_criteria import should_abort_generation
...
class SGLangHttpServer:
    ...
    async def generate(
        ...
        # Enable repetition detection if configured
        enable_repetition_detection = self.config.get("enable_repetition_detection", False)
        tokenizer = self.model_config.tokenizer if self.model_config.tokenizer is not None else None
        
        # Get detection parameters from config (using flat structure from RolloutConfig)
        check_interval = self.config.get("repetition_detection_check_interval", 50)
        max_tokens_after_box = self.config.get("repetition_detection_max_tokens_after_box", 200)
        detect_multiple_boxes = self.config.get("repetition_detection_detect_multiple_boxes", True)
        detect_rethink_patterns = self.config.get("repetition_detection_detect_rethink_patterns", True)
        max_box_length = self.config.get("repetition_detection_max_box_length", 500)
        
        output = None
        aborted = False
        abort_reason = None
        
        # Stream generation to check for repetition
        async for chunk in self.tokenizer_manager.generate_request(generate_request, None):
            output = chunk
            
            # Check repetition periodically during generation (only if not already aborted)
            if enable_repetition_detection and tokenizer is not None and not aborted:
                current_token_ids = output.get("output_ids", [])
                
                # Only check at intervals to reduce overhead
                if len(current_token_ids) % check_interval == 0 and len(current_token_ids) > 0:
                    try:
                        # Decode current output
                        response_text = tokenizer.decode(current_token_ids, skip_special_tokens=True)
                        
                        # Check if we should abort
                        should_abort, reason = should_abort_generation(
                            response_text=response_text,
                            tokens_generated=len(current_token_ids),
                        ...
                        if should_abort:
                            logger.warning(
                                f"[RepetitionDetector] Aborting generation for request {request_id}: {reason}"
                            )
                        ...
                        logger.warning(f"[RepetitionDetector] Error during repetition check: {e}")
        
        # Extract results from final output
        if return_logprob:
        ...
        # Set stop_reason based on whether generation was aborted
        stop_reason = "aborted" if aborted else "completed"
